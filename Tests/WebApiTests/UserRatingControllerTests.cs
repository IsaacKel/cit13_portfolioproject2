...
{
  public class UserRatingControllerTests : IAsyncDisposable
  {
    private const string UserRatingsApi = "http://localhost:5002/api/userrating";
    private static readonly HttpClient client = new HttpClient();
    private readonly List<string> _createdUserRatingIds = new List<string>();
    ...
    public async Task ApiUserRatings_AddUserRating_ValidData_Created()
    {
      ...
      if (id != null)
      {
        _createdUserRatingIds.Add($"{newRating.UserId}/{id}");
      }
    }
    ...
    public async Task ApiUserRatings_UpdateUserRating_ValidData_NoContent()
    {
      ...
      // Clean up after test
      _createdUserRatingIds.Add($"{newRating.UserId}/{rating?["id"]?.ToString()}");
    }

    public async ValueTask DisposeAsync()
    {
      // Clean up any created search histories
      foreach (var id in _createdUserRatingIds)
      {
        await HelperTest.DeleteData($"{UserRatingsApi}/{id}");
      }
    }
  }
}